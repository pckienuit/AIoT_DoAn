...
def _signal_handler(signum, frame):
    """Handle SIGTERM/SIGINT gracefully — set flag so main loop exits."""
...
def main():
    ...
    # Install signal handlers BEFORE opening hardware resources
    ...
    try:
        ...
        # ============================================================
        # CRITICAL CLEANUP — release hardware resources so they don't
        # stay locked after process exit (prevents camera stream hang
        # on restart without a full power cycle).
        # ============================================================
        print("[main] Shutting down — releasing hardware resources...")

        # 1. Close camera (releases VENC/ISP buffers)
        try:
            cam.close()
        ...
        # 2. Close display (releases framebuffer)
        if disp:
            try:
                disp.close()
        ...
        # 3. Force GC to release NN model references (VENC/NPU memory)
        # Note: we don't `del FACE_DET` etc. because they are module-level
        # globals — using `del` inside main() would cause Python to treat them
        # as locals throughout the function, breaking the initial reference.
        gc.collect()

...
MODEL_OUT      = "models/checkpoints/face_recognize_arcface_ft.pth"
RESUME_PATH    = "models/checkpoints/face_recognize_arcface.pth"
EMBEDDING_SIZE = 128      # 128D cho nhẹ, 512D cho chính xác cao hơn
BATCH_SIZE     = 128
EPOCHS         = 30       # Train thêm 30 epoch
LR             = 0.005    # Warm restart với LR nhỏ hơn 10 lần (0.005)
WEIGHT_DECAY   = 5e-4
...
def train():
    ...
    if RESUME and os.path.exists(RESUME_PATH):
        print(f"\n🔄 Phase 2: Resume weights từ checkpoint: {RESUME_PATH}")
        ...
        # KHÔNG load optimizer + scheduler để Reset Learning Rate (Warm Restart)
        
        # 4. Khôi phục metadata (giữ best_lfw nhưng reset epoch)
        start_epoch = 0
        best_lfw    = ckpt.get("lfw_acc", 0.0)
        print(f"   ✅ Đã load weights | LFW cũ={best_lfw*100:.2f}% | Reset LR hiện tại={optimizer.param_groups[0]['lr']:.2e}")
    else:

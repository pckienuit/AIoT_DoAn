...
class CelebADataset(Dataset):
    ...
    def _make_crop(self, image_full: np.ndarray, bx: float, by: float,
        ...
        Unlike zero-padding, replicated pixels preserve edge texture and prevent
        landmark coordinates from being clipped to 0/1 due to black regions.
        """
        CROP_SCALE = 2.14
        ...
        # How much we go beyond each boundary
        pad_left   = max(0, -crop_x)
        pad_top    = max(0, -crop_y)
        pad_right  = max(0, (crop_x + crop_w) - W)
        pad_bottom = max(0, (crop_y + crop_h) - H)

        # Replicate edge pixels to fill out-of-bounds regions instead of zero-padding
        if pad_left or pad_top or pad_right or pad_bottom:
            ...
            # Adjust crop origin since image is now padded
            crop_x += pad_left
            crop_y += pad_top
